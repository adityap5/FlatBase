...
// ─────────────────────────────────────────────────────────────
// Imperative API functions (used by components that can't use hooks)
// ─────────────────────────────────────────────────────────────
...
export const getSellerBookings = (sellerId) =>
  client.query({ query: GET_SELLER_BOOKINGS, variables: { sellerId }, fetchPolicy: 'network-only' });

export const getSellerAnalytics = (sellerId) =>
  client.query({ query: GET_SELLER_ANALYTICS, variables: { sellerId }, fetchPolicy: 'network-only' });

export const getPopularFlats = () =>
  client.query({ query: GET_POPULAR_FLATS, fetchPolicy: 'network-only' });

export const getPopularCities = () =>
  client.query({ query: GET_POPULAR_CITIES, fetchPolicy: 'network-only' });

export const getFlatReviews = (flatId) =>
  client.query({ query: GET_FLAT_REVIEWS, variables: { flatId }, fetchPolicy: 'network-only' });

export const getUser = (id) =>
  client.query({ query: GET_USER, variables: { id }, fetchPolicy: 'network-only' });

export const updateSellerProfile = (profileData) =>
  client.mutate({ mutation: UPDATE_SELLER_PROFILE, variables: profileData });

export const addReview = (reviewData) =>
  client.mutate({ mutation: ADD_REVIEW, variables: reviewData });
